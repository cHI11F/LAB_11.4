...
void savePhoneBookToFile(const map<string, Employee>& phoneBook) {
    ...
    for (const auto& entry : phoneBook) {
        fout.write(reinterpret_cast<const char*>(&entry.second), sizeof(Employee));
    }
...
void readPhoneBookFromFile(map<string, Employee>& phoneBook) {
    ...
    Employee employee;
    while (fin.read(reinterpret_cast<char*>(&employee), sizeof(Employee))) {
        phoneBook[employee.phoneNumber] = employee;
    }
